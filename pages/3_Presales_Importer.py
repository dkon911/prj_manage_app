...
if uploaded_file:
    try:
        # Read Excel file
        df = pd.read_excel(uploaded_file, sheet_name="Input Data", header=1)
        df.columns = df.columns.str.strip()

        # Select and rename columns
        df = df[[
            'Deal Name', 'Project Type', 'Deal Amount',
            'Deal Received(MM/DD/YY)', 'Proposal Sent', 
            'Lost/Canceled', 'Won',
        ...
        # Calculate pending_date as days from deal_received_date to proposal_sent_date
        # Clean date columns (do NOT convert pending_date)
        for col in ['deal_received_date', 'proposal_sent_date', 'won_date', 'lost_date']:
            df[col] = pd.to_datetime(df[col], errors='coerce')

        # Calculate pending_date only for deals that are still pending
        mask_pending = df['won_date'].isna() & df['lost_date'].isna()
        df.loc[mask_pending, 'pending_date'] = (datetime.now() - df.loc[mask_pending, 'proposal_sent_date']).dt.days
        ...
        # Add year/quarter
        df['quarter'] = df['deal_received_date'].dt.quarter
        df['year'] = df['deal_received_date'].dt.year
        
        df['month'] = df['deal_received_date'].dt.month
        df['week'] = df['deal_received_date'].dt.isocalendar().week
        df['day'] = df['deal_received_date'].dt.day
